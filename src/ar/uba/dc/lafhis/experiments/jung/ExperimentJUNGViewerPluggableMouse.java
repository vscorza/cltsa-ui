 ...
 */
public class ExperimentJUNGViewerPluggableMouse extends PluggableGraphMouse {
	/**
    ...
    public ExperimentJUNGViewerPluggableMouse() {
        this(1.1f, 1/1.1f);
    }
    public ExperimentJUNGViewerPluggableMouse(float in, float out) {
		this.in = in;
		this.out = out;
		loadPlugins();
	}
    ...
    protected void loadPlugins() {
    	...
    	pickingPlugin = new PickingGraphMousePlugin<ExperimentJUNGStateVertex,ExperimentJUNGTransitionEdge>();
    ...
    implements MouseListener, MouseMotionListener { 
        ...
        public void mousePressed(MouseEvent e) {
	            down = e.getPoint();
	            ExperimentJUNGStateVertex vertex = null;
	            ExperimentJUNGViewer vv = (ExperimentJUNGViewer)e.getSource();
	            ExperimentJUNGNavigator navigator = vv.getNavigator();
	            GraphElementAccessor<ExperimentJUNGStateVertex,ExperimentJUNGTransitionEdge> pickSupport = vv.getPickSupport();
				if(pickSupport != null) {
	                Layout<ExperimentJUNGStateVertex,ExperimentJUNGTransitionEdge> layout = vv.getGraphLayout();
	                // p is the screen point for the mouse event
	                Point2D ip = e.getPoint();
	                @SuppressWarnings("unused")
	                ExperimentJUNGTransitionEdge edge;
	
	                vertex = pickSupport.getVertex(layout, ip.getX(), ip.getY());
	                if(vertex != null) {
	                	if (navigator.getNext().contains(vertex)) {
	                		navigator.navigateTo(vertex);
	                		for (ExperimentJUNGStateVertex v: navigator.getReached()) {
	                			vv.getPickedVertexState().pick(v, false);
	                		}
	                		for (ExperimentJUNGStateVertex v: navigator.getCurrent()) {
    ...
    implements MouseListener, MouseMotionListener  {
        ...
        private void handlePopup(MouseEvent event) {
            final ExperimentJUNGViewer view = (ExperimentJUNGViewer) event.getSource();
            Point2D p = down;
            
            GraphElementAccessor<ExperimentJUNGStateVertex,ExperimentJUNGTransitionEdge> pickSupport = view.getPickSupport();
            if(pickSupport != null) {
                final ExperimentJUNGStateVertex v = pickSupport.getVertex(view.getGraphLayout(), p.getX(), p.getY());
                if(v != null) {
                    ...
                    for(EnumLayout l: ExperimentJUNGCanvas.EnumLayout.values()) {
                    ...
                    final ExperimentJUNGTransitionEdge e = pickSupport.getEdge(view.getGraphLayout(), p.getX(), p.getY());
                    if(e != null) {
                    }
                }
            }
